...
const MovieInfo = ({ label, value }: MovieInfoProps) => {
  return (
    <View className="flex-col items-start justify-center mt-5 px-5">
      ...
      {value && <Text className="text-white text-sm font-bold">{value}</Text>}
    </View>
  );
};
...
const MovieDetails = () => {
  ...
  const { data: movie } = useFetch(() => fetchMovieDetails(id as string), true);
  ...
  return (
    <View className="bg-primary flex-1">
      <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
        ...
        <MovieInfo
          label="Genres"
          value={movie?.genres?.map(({ name }) => name).join(" - ") || "N/A"}
        />
      ...
      >
        <Image
          source={icons.arrow}
          className="size-5 rotate-180 mr-1 mt-0.5"
          tintColor="#fff"
        />
